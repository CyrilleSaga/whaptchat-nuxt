...
const handleLogin = async () => {
  try {
    ...
    const tokenObj = data.value as { token: string };

    localStorage.setItem('token', tokenObj.token);
    localStorage.setItem('username', username.value);

    // Navigate to chat page
    await nuxtRouter.push('/chat');
      ...
      <div class="bg-white dark:bg-gray-800 rounded-xl shadow-lg overflow-hidden">
        ...
        <div class="px-6 py-4  dark:bg-gray-750 border-t  dark:border-gray-700 text-center">
          <p class="text-gray-600 dark:text-gray-400 text-sm">
            ...
            <NuxtLink class="font-medium text-purple-600 hover:text-purple-500 dark:text-purple-400"
                      to="/auth/register">
              Sign up
            </NuxtLink>
          </p>
